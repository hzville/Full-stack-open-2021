...
beforeEach(async () => {
    ...
    testData.listWithManyUsers.forEach(async user => user.password = await bcrypt.hash(user.password, saltRounds))
...
describe('users creation POST test', () => {
    ...
    test('user is not added if username is missing', async () => {
        ...
        expect(response.body).toContain('error: username or password is missing')
    ...
    test('user is not added if password length is under 3', async () => {
        ...
        const usersInDb = await api_contorllers.getAllUsersFromDb()
        expect(response.statusCode).toEqual(400)
        expect(JSON.stringify(response.body)).toMatch(/error: password must be 3 char or longer/)
        expect(usersInDb.length).toEqual(testData.listWithManyUsers.length)
    })
    ...
    test('user can be create if username is unique', async () => {
        ...
        const usersInDb = await api_contorllers.getAllUsersFromDb()
        expect(response.statusCode).toEqual(400)
        expect(JSON.stringify(response.body)).toMatch(/Error, expected `username` to be unique./)
